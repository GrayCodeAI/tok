#!/usr/bin/env node
// tok — shared configuration resolver
//
// Resolution order for default mode:
//   1. TOK_DEFAULT_MODE environment variable
//   2. Config file defaultMode field:
//      - $XDG_CONFIG_HOME/tok/config.json (any platform, if set)
//      - ~/.config/tok/config.json (macOS / Linux fallback)
//      - %APPDATA%\tok\config.json (Windows fallback)
//   3. 'full'
...
function getConfigDir() {
  if (process.env.XDG_CONFIG_HOME) {
    return path.join(process.env.XDG_CONFIG_HOME, 'tok');
  }
  ...
  return path.join(os.homedir(), '.config', 'tok');
}
...
function getDefaultMode() {
  ...
  const envMode = process.env.TOK_DEFAULT_MODE;
...
// flag path (~/.claude/.tok-active) with a symlink to clobber other files.
//
...
function safeWriteFlag(flagPath, content) {
  try {
    ...
    const tempPath = path.join(flagDir, `.tok-active.${process.pid}.${Date.now()}`);
