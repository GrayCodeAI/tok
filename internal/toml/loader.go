...
type LoaderConfig struct {
	ConfigDir  string // Base config directory (default: ~/.config/tok)
	DataDir    string // Base data directory (default: ~/.local/share/tok)
...
// 1. Project-local: .tok/filters.toml (requires trust)
// 2. User-global: ~/.config/tok/filters.toml
...
func (l *Loader) LoadAll(projectDir string) (*FilterRegistry, error) {
	...
	// HOME/.config/tok for compatibility with older setups.
	userFilterPaths := []string{filepath.Join(l.configDir, "filters.toml")}
	if l.useLegacyDir {
		homeDir, err := os.UserHomeDir()
		if err == nil {
			legacyPath := filepath.Join(homeDir, ".config", "tok", "filters.toml")
	...
	if projectDir != "" {
		projectFiltersPath := filepath.Join(projectDir, ".tok", "filters.toml")
		if _, err := os.Stat(projectFiltersPath); err == nil {
			...
			if l.IsTrusted(projectDir) {
				...
				fmt.Fprintf(os.Stderr, "Warning: project filters not loaded (run 'tok trust' to enable)\n")
			}
		}
	}
...
func (l *Loader) saveTrusted() error {
	...
	var lines []string
	lines = append(lines, "# tok Trusted Projects")
	lines = append(lines, "# Generated by 'tok trust' command")
	lines = append(lines, "")
