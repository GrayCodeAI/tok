...
// Embedded hook script - delegating pattern (v2.0)
const rewriteHook = `#!/usr/bin/env bash
# tokman-hook-version: 2.0
# TokMan Delegating Hook — rewrites commands to use tokman for token savings.
# All logic lives in the tokman binary, not this script.
# Requires: tokman >= 0.1.0, jq

if ! command -v jq &>/dev/null; then
  echo "[tokman] WARNING: jq is not installed. Install: https://jqlang.github.io/jq/download/" >&2
  exit 0
fi

if ! command -v tokman &>/dev/null; then
  echo "[tokman] WARNING: tokman not installed or not in PATH. Install: https://github.com/GrayCodeAI/tokman" >&2
  exit 0
fi
...
if [ -n "$TOKMAN_VERSION" ]; then
  ...
  if [ "$MAJOR" -eq 0 ] && [ "$MINOR" -lt 1 ]; then
    echo "[tokman] WARNING: tokman $TOKMAN_VERSION is too old (need >= 0.1.0). Upgrade: brew upgrade tokman" >&2
    exit 0
  fi
fi
...
# Delegate ALL logic to tokman binary (exit code protocol)
...
case $EXIT_CODE in
  0)
    ...
    jq -n \
      ...
      '{"hookSpecificOutput":{"hookEventName":"PreToolUse","permissionDecision":"allow","permissionDecisionReason":"TokMan auto-rewrite for token optimization","updatedInput":$updated}}'
    ;;
  1)
  ...
  2)
    ...
    ;;
  3)
    # Ask rule matched — rewrite but let AI assistant prompt user for confirmation
    ORIGINAL_INPUT=$(echo "$INPUT" | jq -c '.tool_input')
    UPDATED_INPUT=$(echo "$ORIGINAL_INPUT" | jq --arg cmd "$REWRITTEN" '.command = $cmd')
    jq -n \
      --argjson updated "$UPDATED_INPUT" \
